...
function shortcode_blog_posts_container($atts, $content = null, $code) {
	...
	while ( $blog_array->have_posts() ) : $blog_array->the_post();
		...
		if($style == '1'):
				$output .= '<div class="item container '.$class.'  style-'.$style.'">';
					$output .= '<div class="container-inner">';
						...
						if ($excerpt_length > 0){ 
						$output .= '<div class="post-description">'.tmpmela_blog_post_excerpt($excerpt_length).'</div>';}
						$output .= '<div class="read-more"><a href="'.get_permalink().'" title="'.get_the_title().'">'.esc_html__('read more', 'starbella').'</a></div>';
						$output .= '</div></div>';
		...
		if($style == '2'):	
					...
					$output .= '<div class="post-detail">';
					$output .= '<div class="show-date">'.$post_month.'&nbsp;'.$post_day.','.$post_year.'</div>';
					$shorttitle = substr(get_the_title('','',FALSE),0,60);
					$output .= '<div class="post-title"><a href="'.get_permalink().'" title="'.get_the_title().'">'.$shorttitle.'</a></div>';
					$output .= '</div>';
					$output .= '</div>';
	...
	$output .= '<div class="no-result">'.esc_html__('No results found...', 'starbella').'</div>';
	endif;
	return $output;
}
...
function shortcode_cms_block($atts, $content = null, $code) {
	...
	$noimage = '';
		...
		if(!empty($image)) :
			$cmsimg = '<a href="'.$link_url.' " target="'.$target.'"><div class="cms-banner-img"><img src="'.$image.'" alt="'.get_the_title().'"/></div></a>';
		endif;
		if(!empty($text1)) :	
		...
		if(!empty($text3)) :	
				$cmstext3 = '<span class="text3 static-text">'.$text3.'</span>';
		endif;
		if(!empty($link_text)) :	
				$link_text = '<span class="shop-now"><a href="'.$link_url.' " target="'.$target.'" class="link-text">'.$link_text.'</a></span>';
		endif;		
	
		$output .='<div class="cms-banner-item '.$classname.' style-'.$style.' '.$noimage.'"><div class="cms-banner-inner">'.$cmsimg.'<span class="static-wrapper"><span class="cms-text"><span class="static-inner"><span class="banner-info">'.$cmstext1.''.$cmstext2.''.$cmstext3.''.$link_text.'</span></span></span></span></div></div>';
	...
	extract(shortcode_atts(array(					
		...
		$output .='<div class="tmpmela_about_inner image-'.$image_align.'">';
			...
			if(!empty($image))
				...
				$output .='<div class="readmore"><a href="'.$link_url.'" title="'.$link_text.'" target="'.$target.'">'.$link_text.'<i class="fa fa-long-arrow-right"></i></a></div>';
			$output .='</div>';
		$output .='</div></div>';
		return $output;
	}
...
function shortcode_service($atts, $content = null)
{
	extract(shortcode_atts(array(
		...
		$output ='';
    	$service_desc = '';
    	$service_desc = do_shortcode($content);
    	if(!empty($service_desc)):
			$service_desc = '<div class="service-desc"><a href="'.$link_url.'" title="'.$service_title.'" target="'.$target.'">'.$service_desc.'</a></div>';
    	endif;
 		$output .='<div class="service-list service-'.$service_number.'">';
			$output .='<div class="home-service-content">';
					$output.= '<a class="icon-image" href="'.$link_url.'" title="'.$service_title.' '.$target.'"></a>';
					$output .='<div class="content">';
					$output.='<div class="service-title"><a href="'.$link_url.'" title="'.$service_title.'" target="'.$target.'">'.$service_title.'</a></div>';
    				$output.= $service_desc;
					$output .='</div>';
			$output .='</div>';
		$output .='</div>';
...
function shortcode_tmpmela_single_category($atts, $content = null) {	
   extract(shortcode_atts(array(
	...
	$title = '';	
	$category_ids_array = explode(",",$category_name);
	$category_ids_array[] = $category_ids_array;	
	$output = '';
	$output .= '<div class="category_link">';	
	$output .= '<div class="static-inner">' ;
	if(!empty($text1)) :	
		$title = '<span class="text1 static-text">'.$text1.'</span>';		
	endif;
	$output .='<span class="static-title">'.$title.'</span>';
	...
	foreach($categories as $term) {
			$output .= '<li><a href="'.get_term_link($term).'" target="_self">'.$term->name.'</a></li>';	
	}
	
	$output .= '</ul>';
	if(!empty($link_text)) :	
		$link_text = '<a href="'.$link_url.'" class="link-text">'.$link_text.'</a>';		
	endif;	
	if(!empty($link_text)) :
...
function shortcode_portfolio_slider_container($atts, $content = null, $code) {
				...
				$output .= '<div class="product-block single-portfolio '.$class.' '.$layout.'">';
					$output .= '<div class="portfolio-image">';
							...
							$output .= '<div class="block_hover">';
								...
								$output .= '<a href="'.get_permalink().'" title="'.esc_html__('Click to view read more', 'starbella').'" class="icon"><i class="fa fa-link"></i></a>';							
								$output .= '</div>';
							$output .= '</div>';
					$output .= '</div>';
					$output .= '<div class="portfolio-title"><a href="'.get_permalink().'">'.get_the_title().'</div></a>';
					if ($excerpt_length > 0){ 
					$output .= '<div class="portfolio-description">'.tmpmela_excerpt_length_limit($excerpt_length).'</div>';}
					$output .= '<div class="read-more"><a href="'.get_permalink().'" title="'.get_the_title().'">'.esc_html__('read more', 'starbella').'</a></div>';				
				$output .= '</div>';
				$output .= '</div>';
				$i++;
	...
	$output .= '<div class="no-result">'.esc_html__(''.esc_html__('No results found...', 'starbella').'', 'starbella').'</div>';
	endif;
	return $output;
}
...
function shortcode_woo_products_container($atts, $content = null, $code) {
	...
	if($type == "slider") { 
		...
		$output .= '<div class="woo-grid cols-'.$items_per_column.'">';
	}
	 $output .= do_shortcode($content).'</div>';
	if($type == "grid") {  
		$output .=	'<div class="tmpmela-message"><i class="fa fa-frown-o"></i>'.$no_more.'</div>';
		$output .=	'<div class="loadgridlist-wrapper"><button class="woocount loadgridlist">'.esc_html__('More Products', 'starbella').'</button></div>';		
	}
...
function shortcode_title($atts, $content = null) {
   ...
   'classname' =>''
	...
	if(!empty($color))	
		$output .= '<h3 class="'.$size.'-title" style="color:'.$color.';">'.do_shortcode($content).'</h3>';	
	else
		$output .= '<h3 class="'.$size.'-title">'.do_shortcode($content).'</h3>';
	if(!empty($subtitle))	
...
function shortcode_ourteam($atts, $content = null) {
   extract(shortcode_atts(array(
	...
	while ( $team_array->have_posts() ) : $team_array->the_post();
		...
		$src = tmpmela_mr_image_resize($image, 600, 400, true, 't', false);
		if( empty ( $src ) || $src == 'image_not_specified' ):
			$src = get_template_directory_uri()."/images/megnor/placeholder.png";
			$src = tmpmela_mr_image_resize($src, 600, 400, true, 't', false);
		endif;
			...
			$output .= '<div class="staff-description">'.tmpmela_excerpt_length_limit($excerpt_length).'</div>';
	...
	foreach($testimonial_array as $post) : setup_postdata($post);	
		...
		$output .= '<div class="product-block">';
			$output .= '<div class="custom-testimonial-inner">';											
				...
				$output .= '<div class="read-more"><a title="'.get_the_title().'" href="'.get_permalink().'" >'.esc_html__('read more', 'starbella').'</a></div>';
				$output .= '</div></div>';
...
return __( 'Missing Attachment', 'starbella' );
