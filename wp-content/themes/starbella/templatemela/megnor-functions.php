...
{
    /*  General Settings  */
    add_option("tmpmela_logo_image", get_template_directory_uri() . "/images/megnor/logo.png"); // set logo image
    add_option("tmpmela_logo_image_alt", '<span>star</span>bella'); // set logo image alt
    add_option("tmpmela_mob_logo_image", get_template_directory_uri() . "/images/megnor/logo.png"); // set mobile logo image
    add_option("tmpmela_mob_logo_image_alt", '<span>star</span>bella'); // set logo image alt
    ...
    add_option("tmpmela_background_upload", ""); // Default, texture specified image
    ...
    add_option("tmpmela_button_bkg_color", "2fad4a"); // button background color
    ...
    add_option("tmpmela_menu_text_color", "000000"); // menu text color
    add_option("tmpmela_menu_texthover_color", "2fad4a"); // menu text hover color
    /*  Content Settings  */
    ...
    add_option("tmpmela_hoverlink_color", "2FAD4A"); // link hover color
    ...
    add_option("tmpmela_sidebar_text_hover_color", "2FAD4A"); // Sidebar Text Hover color
    /*  Footer Settings  */
    add_option("tmpmela_footer_bkg_color", "F0F0F0"); // footer background color
    add_option("tmpmela_footer_background_upload", get_stylesheet_directory_uri() . "/images/megnor/footer-bg.png");//footer background image
    add_option("tmpmela_footer_back_repeat", "no-repeat");//background repeat
    add_option("tmpmela_footer_back_position", "bottom+left");//background position
    add_option("tmpmela_footer_back_attachment", "scroll"); //background attachment
    add_option("tmpmela_footerlink_color", "000000"); // footer link text color
    add_option("tmpmela_footerhoverlink_color", "2FAD4A"); // footer link hover text color
    add_option("tmpmela_footerfont", 'Roboto'); // footer google font
    add_option("tmpmela_footerfont_other", 'Arial'); // footer specified font
    add_option("tmpmela_footer_slog", 'Templatemela, All Rights Reserved.'); // copyright statement : Theme By templatemela
    /* Product setting */
...
{
    if (trim(get_option('tmpmela_logo_image_alt')) != '') $logo_alt = esc_attr(get_option('tmpmela_logo_image_alt')); else $logo_alt = esc_attr(get_bloginfo('name', 'display'));
    if (trim(get_option('tmpmela_logo_image')) != '') {
        echo '<img alt="' . esc_attr(get_option('tmpmela_logo_image_alt')) . '" src="' . esc_attr(get_option('tmpmela_logo_image')) . '" />';
    } else {
        echo '<img alt="' . esc_attr(get_option('tmpmela_logo_image_alt')) . '" src=" ' . get_template_directory_uri() . '/images/megnor/logo.png">';
    }
}
function tmpmela_get_mob_logo()
{
    if (trim(get_option('tmpmela_mob_logo_image_alt')) != '') $logo_alt = esc_attr(get_option('tmpmela_mob_logo_image_alt')); else $logo_alt = esc_attr(get_bloginfo('name', 'display'));
    if (trim(get_option('tmpmela_mob_logo_image')) != '') {
        echo '<img alt="' . esc_attr(get_option('tmpmela_mob_logo_image_alt')) . '" src="' . get_option('tmpmela_mob_logo_image') . '" />';
    } else {
        echo '<img alt="' . esc_attr(get_option('tmpmela_mob_logo_image_alt')) . '" src=" ' . get_template_directory_uri() . '/images/megnor/mob-logo.png">';
    }
}
function tmpmela_service_contact()
{
    echo '<div class="contact-block"><div class="contact-inner"><span class="contact-icon"></span><span class="label">' . get_option('tmpmela_contact_label') . '</span></div>&nbsp;<div class="contact-inner number">' . get_option('tmpmela_contact_number') . '</div></div>';
}
...
function tmpmela_admin_scripts()
{
    ...
    wp_enqueue_script('easytabs-min', get_template_directory_uri() . '/js/megnor/admin/jquery.easytabs.min.js');
 ...
 */
function tmpmela_continue_reading_link()
{
    return ' <a class="read-more-link" href="' . esc_url(get_permalink()) . '">' . esc_html__('Read More', 'starbella') . '</a>';
}
/**
 * Replaces "[...]" (appended to automatically generated excerpts) with an ellipsis and tmpmela_continue_reading_link().
 * To override this in a child theme, remove the filter and add your own
 * function tied to the excerpt_more filter hook.
 * @return string An ellipsis
 */
function tmpmela_auto_excerpt_more($more)
{
    return '&hellip;' . tmpmela_continue_reading_link();
}
add_filter('excerpt_more', 'tmpmela_auto_excerpt_more');
/**
 ...
 */
function tmpmela_custom_excerpt_more($output)
{
    if (has_excerpt() && !is_attachment()) {
        $output = '&hellip;' . tmpmela_continue_reading_link();
    }
    return $output;
}
/**
...
function tmpmela_print_images_thumb($src, $alttext, $width = 200, $height = 200, $align = 'left')
{
...
function tmpmela_excerpt($limit)
{
    ...
    if (count($excerpt) >= $limit) {
        array_pop($excerpt);
        $excerpt = implode(" ", $excerpt) . '&nbsp;.<div class="read-more"><a class="read-more-link" href="' . esc_url(get_permalink()) . '">' . esc_html__('Read More', 'starbella') . '</a></div>';
    } else {
...
function tmpmela_load_fonts()
{
    ...
    */
    $Poppins = _x('on', 'Poppins font: on or off', 'starbella');
    $Roboto = _x('on', 'Roboto font: on or off', 'starbella');
    $Intro = _x('on', 'Intro font: on or off', 'starbella');
    if ('off' !== $Poppins || 'off' !== $Intro || 'off' !== $Roboto) {
        $font_families = array();
        if ('off' !== $Poppins) {
        ...
        if ('off' !== $Intro) {
            $font_families[] = 'Intro:200,300,600,700';
        }
        $query_args = array(
...
if (!function_exists('tmpmela_load_scripts')) :
    function tmpmela_load_scripts()
    {
        ...
        wp_enqueue_script('jtree-min', get_template_directory_uri() . '/js/megnor/jquery.treeview.js', array(), '', false);
        wp_enqueue_script('countdown-min', get_template_directory_uri() . '/js/megnor/jquery.countdown.min.js', array(), '', false);
        ?>
        <?php
        wp_enqueue_script( 'tmpmela-html5', get_template_directory_uri() . '/js/html5.js', array(), '', false);
        wp_script_add_data( 'tmpmela-html5', 'conditional', 'lt IE 9' );
        ?>
...
function tmpmela_search_form($form)
{
    ...
    <div><label class="screen-reader-text" for="s">' . esc_attr__('Search for:', 'starbella') . '</label>
	<input class="search-field" type="text" placeholder="' . esc_attr__('Search', 'starbella') . '" value="' . get_search_query() . '" name="s" id="s" />
    <input class="search-submit" type="submit" id="searchsubmit" value="' . esc_attr__('Go', 'starbella') . '" />
    </div>
    </form>';
    return $form;
}
        ...
        switch ($comment->comment_type) :
            ...
            case 'trackback' :
                ...
                <li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
                    <p>
                        <?php esc_html_e('Pingback:', 'starbella'); ?>
                        <?php comment_author_link(); ?>
                        <?php edit_comment_link(esc_html__('(Edit)', 'starbella'), '<span class="edit-link">', '</span>'); ?>
                    </p>
                </li>
                <?php
                break;
                ...
                <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
                    <div id="comment-<?php comment_ID(); ?>" class="comment-body">
                        ...
                        <div class="author-content">
                            ...
                            <?php edit_comment_link(esc_html__('Edit', 'starbella'), ' '); ?>
                            <div class="clearfix"></div>
                            <abbr class="published" title="<?php echo get_the_title() ?>"><?php printf(esc_html__('%1$s at %2$s', 'starbella'), get_comment_date(), get_comment_time()); ?></abbr>
                        </div>
                        ...
                        <?php if ($comment->comment_approved == '0') : ?>
                            <em class="comment-awaiting-moderation">
                                <?php esc_html_e('Your comment is awaiting moderation.', 'starbella'); ?>
                            </em> <br/>
                        <?php endif; ?>
                    </div>
                    <!-- #comment-##  -->
...
if (!function_exists('tmpmela_sticky_post')) :
    function tmpmela_sticky_post()
    {
        if (is_sticky() && is_home() && !is_paged())
            echo '<div class="meta-inner"><span class="sticky-post">' . esc_html__('Sticky', 'starbella') . '</span></div>';
    }
endif;
if (!function_exists('tmpmela_categories_links')) :
    function tmpmela_categories_links()
    {
        ...
        $categories_list = get_the_category_list(esc_html__(', ', 'starbella'));
        if ($categories_list) {
...
if (!function_exists('tmpmela_tags_links')) :
    ...
    {
        // Translators: used between list items, there is a space after the comma.
        $tag_list = get_the_tag_list('', esc_html__(', ', 'starbella'));
        if ($tag_list) {
...
if (!function_exists('tmpmela_author_link')) :
    function tmpmela_author_link()
    {
        ...
        if ('post' == get_post_type()) {
            printf('<div class="meta-inner"><span class="author vcard"><i class="fa fa-pencil-square-o"></i><a class="url fn n" href="%1$s" title="%2$s" rel="author">' . esc_html__("by ", 'starbella') . '%3$s</a></span></div>',
                esc_url(get_author_posts_url(get_the_author_meta('ID'))),
                esc_attr(sprintf(esc_html__('View all posts by %s', 'starbella'), get_the_author())),
                get_the_author()
            );
        }
    }
...
if (!function_exists('tmpmela_comments_link')) :
    function tmpmela_comments_link()
    {
        ...
        if (comments_open() && !is_single()) :
            ...
            comments_popup_link(esc_html__('Leave a Comment', 'starbella'), esc_html__('1 Comment', 'starbella'), esc_html__('% Comments', 'starbella'));
            echo '</span></div>';
        endif;
    }
endif;
if (!function_exists('tmpmela_entry_date')) :
    ...
    function tmpmela_entry_date($echo = true)
    {
        if (has_post_format(array('chat', 'status')))
            $format_prefix = _x('%1$s on %2$s', '1: post format name. 2: date', 'starbella');
        else
            $format_prefix = '%2$s';
...
<time class="entry-date" datetime="%3$s">%4$s</time></a></span></div>',
            ...
            esc_attr(sprintf(esc_html__('Permalink to %s', 'starbella'), the_title_attribute('echo=0'))),
...
if (!function_exists('tmpmela_post_entry_date')) :
    function tmpmela_post_entry_date()
    {
        ...
        $day = get_the_date('d');
        $year = get_the_date('Y');
        $date = '<div class="entry-date"><div class="month">' . $month . '</div>&nbsp;<div class="day">' . $day . ',</div>&nbsp;<div class="year">' . $year . '</div></div>';
        echo wp_kses_post($date);
        return $date;
    }
endif;
if (!function_exists('tmpmela_posts_short_description')) :
    function tmpmela_posts_short_description()
    {
        ...
        if (empty($tmpmela_posts_short_description))
            $tmpmela_posts_short_description = tmpmela_excerpt(45);
    ...
    function tmpmela_shortcode_paging_nav()
    {
        ...
        if ($GLOBALS['wp_query']->max_num_pages > 1) {
            ...
            $links = paginate_links(array(
                ...
                'prev_text' => wp_kses(__('<i class="fa fa-angle-left"></i>', 'starbella'), tmpmela_allowed_html()),
                'next_text' => wp_kses(__('<i class="fa fa-angle-right"></i>', 'starbella'), tmpmela_allowed_html()),
            ));
            if ($links) :
...
add_image_size('tmpmela-blog-posts-list', 1170, 823, true);
...
if (!function_exists('tmpmela_out_of_stock')) :
    function tmpmela_out_of_stock()
    {
        ...
        if (!$product->is_in_stock()) {
            echo '<div class="soldout_wrapper"><span class="soldout">' . esc_html__('SOLD OUT', 'starbella') . '</span></div>';
        }
    }
endif;
...
{
    if ($rating > 0) {
        $title = sprintf(__('Rated %s out of 5', 'starbella'), $rating);
    } else {
        $title = 'Not yet rated';
        $rating = 0;
    }
    $rating_html = '<div class="star-rating" title="' . $title . '">';
    $rating_html .= '<span style="width:' . (($rating / 5) * 100) . '%"><strong class="rating">' . $rating . '</strong> ' . __('out of 5', 'starbella') . '</span>';
    $rating_html .= '</div>';
    return "" ; //$rating_html;
}
...
function tmpmela_update_comment_fields($fields)
{
    ...
    $label = $req ? '*' : ' ' . __('(optional)', 'starbella');
    $aria_req = $req ? "aria-required='true'" : '';
    $fields['author'] =
        '<p class="comment-form-author comment-block">
<input id="author" name="author" type="text" placeholder="' . esc_attr__("Name*", "starbella") . '" value="' . esc_attr($commenter['comment_author']) .
        '" size="30" ' . $aria_req . ' />
</p>';
    $fields['email'] =
        '<p class="comment-form-email comment-block">
<input id="email" name="email" type="email" placeholder="' . esc_attr__("E-mail*", "starbella") . '" value="' . esc_attr($commenter['comment_author_email']) .
        '" size="30" ' . $aria_req . ' />
</p>';
    $fields['url'] =
        '<p class="comment-form-url comment-block">
<input id="url" name="url" type="url"  placeholder="' . esc_attr__("Website", "starbella") . '" value="' . esc_attr($commenter['comment_author_url']) .
        '" size="30" />
</p>';
    return $fields;
}
