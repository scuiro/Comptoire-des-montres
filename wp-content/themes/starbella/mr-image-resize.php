 ...
 */
add_action('delete_attachment', 'tmpmela_mr_delete_resized_images');
function tmpmela_mr_image_resize($url, $width = null, $height = null, $crop = true, $align = 'c', $retina = false)
{
    global $wpdb;
    // Get common vars
    $args = func_get_args();
    $common = tmpmela_mr_common_info($args);
...
function tmpmela_mr_common_info($args)
{
    // Unpack arguments
    ...
    if (preg_match('/\/[0-9]{4}\/[0-9]{2}\/.+$/', $urlinfo['path'], $matches)) {
        ...
        $file_path = ABSPATH . str_replace(get_template_directory() . '/', '', strstr($pathinfo['path'], $uploads_dir));
...
function tmpmela_mr_delete_resized_images($post_id)
{
