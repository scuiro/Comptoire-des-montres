    ...
    function tmpmela_paging_nav()
    {
        ...
        if ($links) :
            ...
            <nav class="navigation paging-navigation" role="navigation">
                <h3 class="screen-reader-text"><?php esc_html_e('Posts navigation', 'starbella'); ?></h3>
...
if (!function_exists('tmpmela_post_nav')) :
    ...
    function tmpmela_post_nav()
    {
        ...
        <nav class="navigation post-navigation" role="navigation">
            <h3 class="screen-reader-text"><?php esc_html_e('Post navigation', 'starbella'); ?></h3>
            <div class="nav-links">
                <?php
                if (is_attachment()) :
                    previous_post_link('%link', wp_kses(__('<span class="meta-nav">Published In</span>%title', 'starbella'), tmpmela_allowed_html()));
                else :
                    previous_post_link('%link', wp_kses(__('<i class="fa fa-chevron-left"></i>Previous Post', 'starbella'), tmpmela_allowed_html()));
                    next_post_link('%link', wp_kses(__('Next Post<i class="fa fa-chevron-right"></i>', 'starbella'), tmpmela_allowed_html()));
                endif;
                ?>
            </div><!-- .nav-links -->
        </nav><!-- .navigation -->
...
if (!function_exists('tmpmela_posted_on')) :
    ...
    function tmpmela_posted_on()
    {
        if (is_sticky() && is_home() && !is_paged()) {
            echo '<span class="featured-post">' . esc_html__('Sticky', 'starbella') . '</span>';
        }
