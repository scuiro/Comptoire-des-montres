...
function tmpmela_setup() {
	...
	* replace to change 'starbella' to the name of your theme in all
	* template files.
	*/
	load_theme_textdomain( 'starbella', get_template_directory() . '/languages' );
	/*
	...
	register_nav_menus( array(
		 'primary'   => esc_html__( 'TM Header Navigation', 'starbella' ),
		 'header-menu'   => esc_html__( 'TM Header Account Links', 'starbella' ),
		 'footer-menu'   => esc_html__( 'TM Footer Navigation', 'starbella' ),
	) );
	/*
...
function tmpmela_widgets_init() {
	register_sidebar( array(
		'name' => esc_html__( 'Main Sidebar', 'starbella' ),
		'id' => 'sidebar-1',
		'description' => esc_html__( 'Appears on posts and pages except the optional Front Page template, which has its own widgets', 'starbella' ),
...
function tmpmela_fonts_url() {
	 ...
	 */
	$source_sans_pro = _x( 'on', 'Source Sans Pro font: on or off', 'starbella' );
	/* Translators: If there are characters in your language that are not
	 * supported by Bitter, translate this to 'off'. Do not translate into your
	 * own language.
	 */
	$bitter = _x( 'on', 'Bitter font: on or off', 'starbella' );
...
<div class="contributor">
    ...
    <div class="contributor-avatar"><?php echo esc_attr(get_avatar( $contributor_id, 132 )); ?></div>
        <div class="contributor-summary">
             ...
             <a class="contributor-posts-link" href="<?php echo esc_url( get_author_posts_url( $contributor_id ) ); ?>"> <?php printf( _n( '%d Article', '%d Articles', $post_count, 'starbella' ), $post_count ); ?> </a>
...
function tmpmela_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'starbella' ), max( $paged, $page ) );
	}
	return $title;
}
...
<div class="related_posts_outer">
    ...
    <h3 class="related-blog-post-title"> <?php echo esc_html_e('Related Posts','starbella');  ?></h3>
    </div>
...
function tmpmela_sampledata_import_files() {
    return array(
        array(
            'import_file_name'             => __( 'Default Demo', 'starbella' ),
            'local_import_file'            => trailingslashit( get_stylesheet_directory() ) . '/demo-content/default/starbella.wordpress.xml',
            'local_import_customizer_file' => trailingslashit( get_stylesheet_directory() ) . '/demo-content/default/starbella_customizer_export.dat',
			         'local_import_widget_file'     => trailingslashit( get_stylesheet_directory() ) . '/demo-content/default/starbella_widgets_settings.wie',
...
function tmpmela_sampledata_after_import($selected_import) {
        ...
        $top_menu = get_term_by('name', 'Header Account Links', 'nav_menu');
